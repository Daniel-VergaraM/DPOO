...
public class ProductoAjustado implements Producto {
	...
	public boolean agregarIngrediente(Ingrediente ing) throws NullPointerException {
		if (ing == null) {
			throw new NullPointerException("El ingrediente no puede ser nulo.");
		}
		return agregados.add(ing);
	}

	public boolean removerIngrediente(String nombre) throws NullPointerException {
		if (nombre == null) {
			throw new NullPointerException("El nombre del ingrediente no puede ser nulo.");
		}
		Ingrediente i = new Ingrediente(nombre, 0);
		return eliminados.add(i);
	}
}
